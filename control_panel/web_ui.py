        ...
        try:
            ...
            if template_dir.exists() and static_dir.exists():
                package_template_dir = (
                    Path(__file__).resolve().parent / "templates" / "web"
                )
            ...
            print(
                f"Warning: Using templates and static files from current directory due to error: {e}"
            )
    else:
        # In local mode, we need to use the local file paths
        template_folder = str(
            Path(__file__).resolve().parent.parent / "templates" / "web"
        )
...
def service_control(name, action):
    if action in ["start", "stop", "restart"]:
        ...
        if not success:
            return redirect(
                url_for(
                    "index", action=action, service=name, status="error", message=error
                )
            )
    elif action == "enable":
        config = load_config()
...
def delete_service(name):
    ...
    if not success:
        return redirect(
            url_for(
                "index", action="delete", service=name, status="error", message=error
            )
        )
