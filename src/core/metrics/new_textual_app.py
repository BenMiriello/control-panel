...
from src.core.metrics.hardware_detector import get_hardware_info
from src.core.metrics.service_metrics import get_all_service_metrics
from src.core.system_metrics import get_all_metrics
...
class ServicesTableWidget(Static):
    ...
    def update_services(self, service_metrics: List[Dict]):
        ...
        for service_index, service in enumerate(service_metrics):
            ...
            is_selected = current_display_row - 3 == self.cursor_row
            ...
            # Track which rows are selectable
            self.row_to_service_map[current_display_row - 3] = service_index
            self.selectable_rows.append(current_display_row - 3)
    ...
    def move_cursor_up(self):
        ...
        if self.cursor_row > 0:
            self.cursor_row -= 1
            # Immediately update display to show new selection
...
class MetricsApp(App):
    ...
    def __init__(self, refresh_interval: float = 3.0, **kwargs):
        super().__init__(**kwargs)
    ...
    def action_service_actions(self) -> None:
        ...
        if service_index < len(self.services_widget.service_metrics):
            ...
            # Show simple notification with available actions
            action_text = f"Actions for {service_name} ({status}): " + " | ".join(
                actions
            )
            self.notify(action_text, title="Service Actions", timeout=3)


def run_metrics_dashboard(refresh_interval: float = 3.0):
