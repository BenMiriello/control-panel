    ...
    try:
        ...
        try:
            ...
            for gpu_id, gpu_info in data.items():
                if (
                    "GPU use" in gpu_info
                    and "Memory use" in gpu_info
                    and "Temperature" in gpu_info
                ):
                    gpu_data.append(
                        {
                ...
                for chip, values in data.items():
                    ...
                    for key, items in values.items():
                        if isinstance(items, dict) and any(
                            "input" in k for k in items.keys()
                        ):
                            for temp_key, temp_val in items.items():
                                if "input" in temp_key:
                                    temps.append(
                                        {"zone": chip, "type": key, "temp": temp_val}
                                    )
            except (
...
def get_all_metrics():
    ...
    return {
        "cpu": {"usage": get_cpu_usage(), "temperature": get_cpu_temperature()},
